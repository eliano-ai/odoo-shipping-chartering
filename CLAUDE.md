...
| Compute field TIDAK `store=True` dipakai sebagai `type="measure"` di `<pivot>`/`<graph>` view | **`aggregator=` attribute SENDIRIAN TIDAK CUKUP** — Odoo 19 (`Field._description_aggregator()`, `odoo/orm/fields.py`) selalu coba validasi aggregator lewat SQL `SELECT` beneran (`_read_group_select`) sebelum mengekspos-nya ke `fields_get()`; field non-stored SELALU gagal di validasi itu (`ValueError: Cannot convert ... to SQL because it is not stored`) dan `aggregator` di-fallback jadi `None` diam-diam — pivot tetap error `No aggregate function has been provided for the measure 'X'` walau `aggregator='sum'` sudah ditulis di kode. **Satu-satunya fix**: `store=True` sungguhan (bukan cuma `aggregator=`). Kalau compute-nya bergantung pada `search()` lintas-model (bukan `@api.depends` field lokal/related biasa — lihat baris "compute field depend search() lintas model" di bawah), `store=True` membawa risiko staleness yang sama; kalau semua dependency-nya field lokal/related di model yang sama, `store=True` aman tanpa risiko tambahan. Ditemukan 2026-07-08 dari laporan error user (`vessel.vessel.budget.line.actual_amount` di pivot "Realisasi vs Budget", `vessel.voyage.pnl.revenue_variance_pct`/`cost_variance_pct` kena pola sama meski belum pernah dilaporkan error). **Cek proaktif kalau menambah field measure baru ke `<pivot>`/`<graph>`**: pastikan field itu `store=True`, jangan asumsi `aggregator=` cukup. |
| `widget="percentage"` di view untuk field yang compute-nya sudah `* 100.0` (mis. `variance_pct = (variance / total) * 100.0`, nilai tersimpan "50.0" utk 50%) | **`widget="percentage"` SELALU mengalikan value dengan 100 lagi untuk display** (`formatPercentage`, asumsi value tersimpan sebagai FRAKSI 0-1) — kalau compute Python-nya SUDAH `* 100.0` (pola dominan project ini, field bernama `*_pct` selalu whole-number percent, BUKAN fraksi), hasilnya 50.0 → tampil "5000%" (dikali 100 dua kali). Odoo tidak punya widget variant untuk skip perkalian ini. **Fix**: hapus `widget="percentage"`, biarkan tampil plain float — label field sudah ada "(%)" jadi tidak hilang makna. Ditemukan 2026-07-06 dari laporan error user (`vessel.vessel.budget.line.variance_pct` nunjukkan 5000% bukan 50%) — mengenai **7 field sekaligus** (`vessel.vessel.budget.line.variance_pct`, `vessel.vessel.pnl.utilization_pct`, `vessel.voyage.pnl.revenue_variance_pct`/`cost_variance_pct`, `vessel.bunker.quote.price_vs_market_pct`, `vessel.bunker.rob.reconciliation.variance_pct`, `vessel.bunker.survey.variance_pct`) karena semua reuse pola compute `* 100.0` yang sama — sama seperti gotcha `default_` di atas, **tidak pernah ketahuan karena hanya diverifikasi lewat RPC/shell (baca raw value), tidak pernah benar-benar dilihat widget-nya di browser**. **Cek proaktif**: field `*_pct`/`*_percent` baru — kalau compute-nya `* 100.0`, JANGAN pasang `widget="percentage"`. |
| `fields.Integer` yang representasi angka non-kuantitas (mis. tahun `2026`) ditampilkan di form/list tanpa opsi | **`IntegerField` widget default mengelompokkan ribuan** (`formatInteger`, `enable_formatting` default `True`) — field seperti `year`/`period_year` (Integer, bukan Monetary/Float biasa) tampil "2,026" alih-alih "2026". Fix: `<field name="year" options="{'enable_formatting': False}"/>`. Ditemukan 2026-07-08 bareng dengan temuan `widget="percentage"` di atas (laporan error/screenshot user yang sama) — pola serupa: field Integer yang secara semantik BUKAN kuantitas (ID, tahun, nomor referensi) rawan salah kena default formatting number kalau tidak eksplisit di-opt-out. |
